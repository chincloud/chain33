...
const TxnSeqKey = contextKey("TXN_SEQUENCE")

type TxnSeqState struct {
	Sequence *int32
}
...
func (c *core) buildMultipleValue(state *coreState, key string) map[string][]byte {
	values := make(map[string][]byte, 1)
...
func (c *core) DoTransaction(ctx context.Context, db ycsb.DB) error {
	...
	seqState := ctx.Value(TxnSeqKey).(*TxnSeqState)
	seq := int(atomic.AddInt32(seqState.Sequence, 1))

	operation := operationType(c.operationChooser.Next(r))
	switch operation {
	case read:
		return c.doTransactionRead(ctx, db, state, seq)
	case update:
		return c.doTransactionUpdate(ctx, db, state, seq)
	case insert:
		return c.doTransactionInsert(ctx, db, state)
	case scan:
		return c.doTransactionScan(ctx, db, state)
	default:
		return c.doTransactionReadModifyWrite(ctx, db, state, seq)
	}
}
...
func (c *core) doTransactionRead(ctx context.Context, db ycsb.DB, state *coreState, seq int) error {
	r := state.r
	...
	values, err := db.Read(ctx, strconv.Itoa(seq), keyName, fields)
	if err != nil {
		return err
	}
...
func (c *core) doTransactionReadModifyWrite(ctx context.Context, db ycsb.DB, state *coreState, seq int) error {
	start := time.Now()
	defer func() {
	...
	if c.writeAllFields {
		...
		values = c.buildMultipleValue(state, keyName)
	}
	defer c.putValues(values)

	readValues, err := db.Read(ctx, strconv.Itoa(seq), keyName, fields)
	if err != nil {
		return err
	}

	if err := db.Update(ctx, strconv.Itoa(seq), keyName, values); err != nil {
		return err
	}
	...
	//key := c.buildKeyName(c.nextKeyNum(state))
	//if err := db.Update(ctx, strconv.Itoa(seq), key, c.buildSingleValue(state, key)); err != nil {
	//	return err
	//}
...
func (c *core) doTransactionUpdate(ctx context.Context, db ycsb.DB, state *coreState, seq int) error {
	keyNum := c.nextKeyNum(state)
	keyName := c.buildKeyName(keyNum)
	...
	//	return err
	//}
	return db.Update(ctx, strconv.Itoa(seq), keyName, values)
}
...
func (coreCreator) Create(p *properties.Properties) (ycsb.Workload, error) {
	...
	case "zipfian":
		...
		c.keyChooser = generator.NewScrambledZipfian(keyrangeLowerBound, keyrangeUpperBound-1, generator.ZipfianConstant)
	case "latest":
